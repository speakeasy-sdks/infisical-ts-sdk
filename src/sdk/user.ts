  ...
  **/
  myOrganizations(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.MyOrganizationsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  myUser(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.MyUserResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
